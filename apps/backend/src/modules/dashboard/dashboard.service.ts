...
export class DashboardService {
  ...
  async getStats(companyId: string, role?: string, userId?: string): Promise<DashboardStatsDto> {
    ...
    try {
      ...
      // For SUPER_ADMIN, query across all companies (system-wide)
      const buildBaseQuery = () => {
        const query = this.ticketRepo.createQueryBuilder('ticket');
        
        if (role === 'SUPER_ADMIN') {
          // No companyId filter for SUPER_ADMIN - system-wide query
        } else {
      ...
      // SUPER_ADMIN: No additional filters - show all tickets system-wide
      // Query already built without companyId filter for SUPER_ADMIN
      ...
      // For SUPER_ADMIN, count across all companies (system-wide)
      const activeUsers =
        role !== 'TENANT'
          ? role === 'SUPER_ADMIN'
      ...
      // Debug logging for ON_HOLD count issue
      if (onHold > 0) {
        console.log(
          `[DashboardService] ON_HOLD count: ${onHold} for companyId: ${companyId}, role: ${role}`,
        );
        // Get actual ON_HOLD tickets for debugging
        const onHoldTickets = await baseQuery
          .clone()
          .andWhere('ticket.status = :status', { status: TicketStatus.ON_HOLD })
          .select(['ticket.id', 'ticket.ticketNumber', 'ticket.status', 'ticket.title'])
          .getMany();
        console.log(
          `[DashboardService] ON_HOLD tickets found:`,
          onHoldTickets.map((t) => ({
            id: t.id,
  ...
  async getAnalytics(
    ...
    try {
      ...
      if (role !== 'SUPER_ADMIN') {
        baseQuery.where('ticket.companyId = :companyId', { companyId });
      }
  ...
  private async calculateTechnicianPerformance(
    ...
    // For SUPER_ADMIN, query across all companies
    const technicianWhere: any = {
      id: In(technicianIds),
    };
    if (role !== 'SUPER_ADMIN' && companyId) {
      technicianWhere.companyId = companyId;
    }
